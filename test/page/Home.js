...
export async function selectFromDropdownAndPrint(option) {
    ...
    const dropdownList = await $(dropdownExample).selectByAttribute('value', `option${option}`);
...
export async function openNewTabAndPrint() {
    ...
    for(var i = 0; i< handles.length; i++){
        if( handles[i]!= parentTab){
            ...
            const pageUrl = await browser.getUrl();
            const pageTitle = await browser.getTitle();
            console.log(`The tab URL is ${pageUrl} and the tab title is ${pageTitle}`);
            await browser.pause(3000);
            await browser.closeWindow();
            break;
        }
...
export async function switchToIframeAndPrint() {
    ...
    const email = await $(`//li[text() = " contact@rahulshettyacademy.com"]`).getText();
